...
let
  ...
  hostsOnNetwork =
    ...
    lib.filterAttrs (
      _: host:
      host.wireguard != null
      && lib.any (iface: iface.network == network) (lib.attrValues host.wireguard.interfaces)
    ) topology;
  ...
  ifaceForNetwork =
    host: network:
    lib.findFirst (iface: iface.network == network) null (lib.attrValues host.wireguard.interfaces);
in
{
  ...
  config = lib.mkIf (cfg.enable && currentHost != null) {
    networking = {
      ...
      useDHCP =
        !currentHost.networkManager && lib.any (icfg: icfg.dhcp) (lib.attrValues currentHost.interfaces);

      interfaces = lib.mkIf (!currentHost.networkManager) (
        lib.mapAttrs (iface: icfg: {
          ipv4.addresses = lib.mkIf (icfg.address != null) [
            {
      ...
      defaultGateway =
        let
          ...
          gateways = lib.filter (g: g != null) (lib.mapAttrsToList (_: icfg: icfg.gateway) staticInterfaces);
        in
      ...
      wireguard.interfaces = lib.mkIf (currentHost.wireguard != null) (
        lib.mapAttrs (wgName: wgIface: {
          ...
          peers =
            let
              others = lib.filterAttrs (name: _: name != currentHostName) (hostsOnNetwork wgIface.network);
            in
            if wgIface.isServer then
              lib.mapAttrsToList (
                name: host:
                let
    ...
    networking.firewall = lib.mkIf (currentHost.wireguard != null) {
      allowedUDPPorts = lib.flatten (
        lib.mapAttrsToList (
          _: wgIface: lib.optional wgIface.isServer wgIface.listenPort
        ) currentHost.wireguard.interfaces
      );
    };
    ...
    services.avahi = {
      enable = true;
      nssmdns4 = true;
      publish = lib.mkIf (!currentHost.networkManager) {
        enable = true;
        hinfo = true;
        addresses = true;
        workstation = true;

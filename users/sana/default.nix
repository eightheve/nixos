{
  ...
  config = lib.mkMerge [
    (lib.mkIf config.myUsers.sana.enable {
      users.users.sana = {
        isNormalUser = true;
        createHome = true;
        description = "二葉さな";
    ...
    (lib.mkIf (config.myUsers.sana.enable && config.myUsers.sana.useHomeManager) {
    ...
    (lib.mkIf (config.myUsers.sana.enable && config.myUsers.sana.useHomeManager && config.myUsers.sana.enableGraphics) {
      ...
      home-manager.users.sana = {
        ...
        homeModules.windowManagers.dwm = lib.mkIf (config.myUsers.sana.windowManager == "dwm") {
          enable = true;
          makeXinitrc = true;
        ...
        homeModules = {
          ...
          fish = {
            ...
            settings = {
              useGitStatus = false;
              promptColors = lib.mkIf (config.networking.hostName == "SATELLITE") (let
